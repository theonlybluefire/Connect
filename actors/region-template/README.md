...
- [ ] Copy this template
- [ ] Change package names in the `package.json`, `actor.json` files (replace `[region]` with the region of the scraper)
- [ ] Create a `input.json` file containing the actors configuration as a json object (see `input_schema.json`)
- [ ] Install a dependencies using `npm install`
- [ ] Set the `REGION` constant to the region of the actor
